...
//	//ssthresh_ = cwnd_;
    ...
    //printf("whx flowid: %d produce %d pkts,%s\n",flow_id,nb,statestr(state_));
...
FullTcpAgent::bufferempty()
{
       ...
       //printf("whx flowid: %d is done\n",flow_id);//can't be invoked anytime.
...
FullTcpAgent::usrclosed()
{
	...
	case TCPS_CLOSING:
//whx
//printf("%s, %d,%lf","whx closing  ",flow_id,now());//can't be invoked anytime.
//whx
...
//printf("whx , %d, state changed from %s to %s\n",  flow_id,statestr(state_), statestr(state));
	state_ = state;
}
...
{
//whx
//printf("%s, %d,%lf"," whx finish",flow_id,now());//can't be invoked anytime.
//whx
...
if (flow_id <10 ) {
          ...
          //     printf("whx  %f, flowid: %d iss:%d curseq:%d,seqno: %d,datalen:%d\n",now(),flow_id,iss_,(int)curseq_,seqno,datalen);
         }
//whx, trace end time of a flow
...
//if(datalen>0)
//if(flow_id<6)
 //   printf("whx  flowid: %d endtime: %f pkts:%d  %d\n",flow_id,now(),(seqno-iss_)/maxseg_,datalen);
//whx
...
//	if ((seqno + datalen) > curseq_ && datalen == 0 && flow_id == 1 ) {
//if ((seqno + datalen) >= curseq_  && flow_id < max_short_flow_id ) {
//if (flow_id <10 ) {
                end_time = now();
  ...
  //             printf("whx  flowid: %d iss:%d,seqno: %d,curseq:%d,datalen:%d\n",flow_id,iss_,seqno,curseq_,t);
    //           printf("whx  flowid: %d iss:%d curseq:%d,seqno: %d,datalen:%d\n",flow_id,iss_,(int)curseq_,seqno,datalen);
        //                printf("%d %d \n",flow_max_cwnd_,(int)cwnd_);
		}
  //       }
//whx, trace end time of a flow
...
//if (tiflags&TH_FIN) {
//printf("whx  %f(%s)in state %s recv'd this packet: ,%d\n", now(), name(), statestr(state_),flow_id);
//prpkt(pkt);
//}
...
/* dodata: this label is in the "real" code.. here only for reference */
	...
	if (tiflags & TH_FIN) {
		 ...
		 * (completion of active close)
                 */
                case TCPS_FIN_WAIT_2:
                       // printf("whx  %f(%s)in state %s recv'd this packet: ,%d\n", now(), name(), statestr(state_),flow_id);//can't be invoked anytime.
                        newstate(TCPS_CLOSED);
			cancel_timers();
                        break;
		}
